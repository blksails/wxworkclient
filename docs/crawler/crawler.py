...
class WeChatWorkAPICrawler:
    ...
    def __init__(
        ...
        self.split_multi_api = (
            split_multi_api  # 是否分割多接口页面（已弃用，由 LLM 自动处理）
        )
        self.failed_docs = []  # 记录失败的文档 ID 和错误信息
        self.route_filter = (
            route_filter or []
        )  # 路由过滤器数组，必须包含所有关键词才通过 - AND 逻辑（如 ["第三方应用开发", "服务端API"]）
        self.route_exclude = (
            route_exclude or []
        )  # 路由排除器数组，包含任意一个就排除 - OR 逻辑（如 ["服务商代开发"]）
        ...
        if self.doc_ids:
            ...
            self.queue = [
                urljoin(base_url, f"/document/path/{doc_id}") for doc_id in self.doc_ids
            ]
    ...
    def crawl(self):
        ...
        if self.resume and self.visited:
            print(
                f"断点续爬模式: 已访问 {len(self.visited)} 个页面，队列中还有 {len(self.queue)} 个待爬取"
            )
        print()
        ...
        try:
            ...
            if self.failed_docs:
                print(
                    f"  - 失败文档: {len(self.failed_docs)} 个（见 .failed_docs.json）"
                )
            print()
            print("重新运行命令继续爬取：")
            ...
            if (self.route_filter or self.route_exclude) and not already_visited:
                ...
                if route_path:
                    ...
                    if self.route_filter:
                        matched = all(
                            filter_keyword in route_path
                            for filter_keyword in self.route_filter
                        )
                        if not matched:
                            missing = [
                                f for f in self.route_filter if f not in route_path
                            ]
                    ...
                    if self.route_exclude:
                        for exclude_keyword in self.route_exclude:
                            if exclude_keyword in route_path:
                                print(
                                    f"  ⊗ 跳过: 包含已排除的 '{exclude_keyword}' 路由"
                                )
                                return
                else:
                    print(f"  ⚠️  未找到路由信息")
            ...
            if not self.doc_ids:
                ...
                for link in soup.find_all("a", href=True):
                    ...
                    if "/document/path/" in href:
                        ...
                        if (
                            urlparse(full_url).netloc == urlparse(self.base_url).netloc
                            and full_url not in self.visited
                            and full_url not in self.queue
                        ):
...
7. **⭐ 数组类型检测**（关键）：
   - **必须从响应示例中推断数组元素类型**
   - 如果数组包含字符串，使用 `string[]`（例如：`["LINK_ID_AAA", "LINK_ID_BBB"]` → `string[]`）
   - 如果数组包含数字，使用 `int32[]` 或 `uint32[]`（例如：`[1, 2, 3]` → `int32[]`）
   - 如果数组包含对象，使用 `object[]` 并展开对象字段（例如：`[{"id": 1, "name": "test"}]` → `object[]`，然后添加 `items[].id` 和 `items[].name`）
   - 如果数组为空或无法推断，使用 `array`
   - **示例**：
     - `"link_id_list": ["LINK_ID_AAA", "LINK_ID_BBB"]` → type: `string[]`
     - `"user_list": ["rocky", "sam"]` → type: `string[]`
     - `"department_list": [1, 2, 3]` → type: `int32[]`
     - `"items": [{"id": 1, "name": "test"}]` → type: `object[]`，并添加 `items[].id` (int32) 和 `items[].name` (string)
8. **代码示例语言识别**：
   - 包含 { } 或 [ ] 的是 json
   - 包含 <xml> 的是 xml
   - 包含 curl 的是 bash
      ...
      "body_params": [
        {
          ...
          "type": "类型（string/int32/uint32/bool/object/string[]/int32[]/object[]）",
          "required": true,
          "description": "说明"
        }
      ],
...
"""
        ...
        try:
            ...
            if len(apis) > 1:
                ...
                for i, api_info in enumerate(apis, 1):
                    ...
                    # 保存单个 API 的 JSON
                    api_json_file = (
                        self.output_dir / f"{path}-{i}-{api_doc.api_name}.json"
                    )
                    ...
                    print(
                        f"    ✓ 提取接口 {i}/{len(apis)}: {api_doc.group_title or api_doc.title}"
                    )
    ...
    def _extract_section_for_api(
        self, soup: BeautifulSoup, heading_element
    ) -> BeautifulSoup:
        ...
        if not api_doc.api_url or not api_doc.method:
            ...
            if not api_doc.method:
                method_match = re.search(
                    r"请求方式[：:]\s*(GET|POST|PUT|DELETE)", page_text, re.IGNORECASE
                )
                if method_match:
            ...
            if not api_doc.api_url:
                url_match = re.search(
                    r"请求地址[：:]\s*(https://qyapi\.weixin\.qq\.com[^\s\n]*)",
                    page_text,
                )
                if url_match:
            ...
            if not api_doc.api_url or not api_doc.method:
                http_match = re.search(
                    r"(GET|POST|PUT|DELETE)\s+(https://qyapi\.weixin\.qq\.com[^\s\n]*)",
                    page_text,
                )
                if http_match:
                    if not api_doc.method:
        ...
        try:
            ...
            if parsed_url.query:
                ...
                for part in query_parts:
                    if "=" in part:
                        ...
                        if key not in existing_names:
                            ...
                            description = self._infer_query_param_description(
                                key, value
                            )
                ...
                if extracted_count > 0:
                    print(
                        f"    → 补充从 URL 提取的 {extracted_count} 个 Query 参数: {[p.name for p in api_doc.query_params[-extracted_count:]]}"
                    )
    ...
    def _infer_query_param_type(self, key: str, value: str) -> str:
        ...
        if any(
            ...
            for keyword in ["limit", "offset", "page", "size", "count", "num"]
        ):
            return "int"
        ...
        for pre_tag in soup.find_all("pre"):
            ...
            if parent_heading:
                ...
                if "<xml" in code or "</xml>" in code:
                    ...
                    language = "bash"
                elif "http" in code.lower() and (
                    "post" in code.lower() or "get" in code.lower()
                ):
                    language = "http"

                example = CodeExample(
                    title=parent_heading, language=language, code=code
                )

                # 分类存储 - 更精确的判断
                if (
                    "请求包体" in heading_text
                    or "请求体" in heading_text
                    or "请求示例" in heading_text
                ):
                    api_doc.request_examples.append(example)
                elif (
                    "返回结果" in heading_text
                    or "响应" in heading_text
                    or "response" in heading_text
                ):
                    api_doc.response_examples.append(example)
    ...
    def _extract_params_from_json_examples(
        self, soup: BeautifulSoup, api_doc: APIDoc, is_request: bool = True
    ) -> Dict[str, str]:
            ...
            if is_match:
                ...
                try:
                    ...
                    if params:
                        ...
                        print(
                            f"    → 从 {param_type}JSON 中推测了 {len(params)} 个参数的类型"
                        )
        ...
        if isinstance(json_obj, dict):
            for key, value in json_obj.items():
                ...
                if isinstance(value, dict):
                    ...
                    param_type = "object"
                    param = Parameter(
                        name=param_name, type=param_type, required=False, description=""
                    )
                    params.append(param)
    ...
    def _extract_parameters(self, soup: BeautifulSoup, api_doc: APIDoc):
        """提取参数表格"""
        # 首先尝试从 JSON 示例中提取参数
        request_json_params = self._extract_params_from_json_examples(
            soup, api_doc, is_request=True
        )
        response_json_params = self._extract_params_from_json_examples(
            soup, api_doc, is_request=False
        )
        ...
        for table in soup.find_all("table"):
            ...
            name_idx = self._find_column_index(
                headers, ["参数", "name", "字段", "field"]
            )
            type_idx = self._find_column_index(headers, ["类型", "type"])
            required_idx = self._find_column_index(
                headers, ["必填", "required", "必须"]
            )
            ...
            for row in rows[1:]:
                ...
                param_name = (
                    cell_texts[name_idx]
                    if name_idx != -1 and name_idx < len(cell_texts)
                    else cell_texts[0]
                )
                param_type = (
                    cell_texts[type_idx]
                    if type_idx != -1 and type_idx < len(cell_texts)
                    else ""
                )
                param_desc = (
                    cell_texts[desc_idx]
                    if desc_idx != -1 and desc_idx < len(cell_texts)
                    else ""
                )
                ...
                if existing_query_param:
                    ...
                    if required_idx != -1 and required_idx < len(cell_texts):
                        ...
                        existing_query_param.required = any(
                            keyword in required_text
                            for keyword in ["是", "yes", "true", "必填", "必须"]
                        )
            ...
            self._classify_parameters(
                params_list, parent_heading, api_doc, has_return_result_before
            )
        ...
        if request_json_params:
            ...
            if json_only_params:
                ...
                print(
                    f"    → 从请求JSON中补充了 {len(json_only_params)} 个表格中不存在的参数"
                )
        ...
        if response_json_params:
            ...
            if json_only_params:
                ...
                print(
                    f"    → 从响应JSON中补充了 {len(json_only_params)} 个表格中不存在的参数"
                )
    ...
    def _classify_parameters(
        ...
        parent_heading: str,
        api_doc: APIDoc,
        has_return_result_before: bool = False,
    ):
        """智能分类参数到请求参数或响应参数"""
        ...
        if "请求参数" in parent_heading or "request" in parent_heading:
            ...
            "响应参数" in parent_heading
            or "response" in parent_heading
            or "返回参数" in parent_heading
        ):
            api_doc.response_params.extend(params_list)
            return
        ...
        ambiguous_names = {"access_token", "list", "page", "status", "code", "message"}
    ...
    def _save_single_markdown_grouped(self, doc: APIDoc, index: int, total: int):
        ...
        try:
            ...
            print(
                f"    → 已保存: {output_file.name} (完整度: {score}/6, {index}/{total})"
            )
        except Exception as e:
            print(f"    ✗ 保存失败: {e}")
    ...
    def _update_index(self):
        ...
        for doc in self.api_docs:
            ...
            lines.append(
                f"- {score_badge} [{doc.title}]({filename}-{score}.md) `[{score}/6]` - {desc}"
            )
    ...
    def _generate_index(self):
        ...
        for doc in sorted_docs:
            ...
            lines.append(
                f"- {score_badge} {method_tag}[{doc.title}]({filename}-{score}.md) `[{score}/6]` - {desc}"
            )
        ...
        print(
            f"  🔴 需完善: {score_stats[3] + score_stats[2] + score_stats[1] + score_stats[0]} 个"
        )
    ...
    def _check_captcha_page(self, soup: BeautifulSoup) -> bool:
        ...
        # 检查常见的验证码元素
        captcha_indicators = ["captcha", "verify", "verification", "验证码", "人机验证"]
...
def main():
    ...
    parser.add_argument(
        "--doc-ids",
        type=str,
        help="指定要爬取的文档 ID，多个用逗号分隔，例如: 90601,90602",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
