...
- **文档地址**: [https://developer.work.weixin.qq.com/document/path/94207](https://developer.work.weixin.qq.com/document/path/94207)
- **文档 ID**: `94207`
- **请求方法**: `POST`
- **接口地址**: `https://qyapi.weixin.qq.com/cgi-bin/checkin/getcheckin_monthdata`
...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| access_token | string | 是 | 调用接口凭证，使用自建应用的Secret获取access_token |
...
| useridlist | string[] | 是 | 不少于1字节 不多于64字节 可填充个数：1 ~ 100 |
...
| datas.base_info.record_type | uint32 | 记录类型：1-固定上下班；2-外出；3-按班次上下班；4-自由签到；5-加班；7-无规则 |
