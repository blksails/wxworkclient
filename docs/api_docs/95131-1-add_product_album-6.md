...
- **文档 ID**: `95131`
- **API 名称**: `add_product_album`
- **请求方法**: `POST`
- **接口地址**: `https://qyapi.weixin.qq.com/cgi-bin/externalcontact/add_product_album`
- **分组信息**: 第 1 个接口，共 5 个
...
| attachments | object[] | 是 | 附件类型，仅支持image，最多不超过9个附件 |
| attachments[].type | string | 是 | 附件类型，仅支持image |
| attachments[].image.media_id | string | 否 | 图片的media_id，仅支持通过上传附件资源接口获得的资源 |
