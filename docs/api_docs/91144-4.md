...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| msg_signature | string | 是 | 从接收消息的URL中获取的msg_signature参数 |
| timestamp | string | 是 | 从接收消息的URL中获取的timestamp参数 |
| nonce | string | 是 | 从接收消息的URL中获取的nonce参数 |
| echostr | string | 是 | 从接收消息的URL中获取的echostr参数。注意，此参数必须是urldecode后的值 |
| sReplyEchoStr | string | 是 | 解密后的明文消息内容，用于回包。注意，必须原样返回，不要做加引号或其它处理 |
...
鉴于加解密算法相对复杂，企业微信提供了算法库。目前已有c++/python/php/java/golang/c#等语言版本。均提供了解密、加密、验证URL三个接口，企业可根据自身需要下载，[下载地址](#10128)。
